...
def replace_words value
   ...
   digits.each { |k, v| value.gsub!(k,v + k)}

   p "before: #{before} and after: #{value}"
   value
end

values = data.map do |value|
   value = replace_words(value) # second puzzle
   (value.scan(/[0-9]/).first + value.scan(/[0-9]/).last).to_i
end
p values.reduce(:+)
