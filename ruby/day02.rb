...
data.each do |game|
  ...
  add_game = true 
  sets.each do |set|
    set.split(",").each do |roll|
      if roll.match?("blue") && roll.to_i > 14
        add_game = false
        break
      elsif roll.match?("red") && roll.to_i > 12
        add_game = false
        break
      elsif roll.match?("green") && roll.to_i > 13
        add_game = false
        break
      end
    end
  ...
  sum_ids = sum_ids + game_number if add_game
end

p "sum of ids: #{sum_ids}"
...
data.each do |game|
  ...
  sets.each do |set|
    set.split(",").each do |roll|
      if roll.match?("blue") 
        max_blue = roll.to_i if roll.to_i > max_blue
      elsif roll.match?("red") 
        max_red = roll.to_i if roll.to_i > max_red
      elsif roll.match?("green") 
        max_green = roll.to_i if roll.to_i > max_green
      end
    end
  end
  sum_of_pow = sum_of_pow + (max_blue * max_red * max_green)
end
p "sum #{sum_of_pow}"
__END__
